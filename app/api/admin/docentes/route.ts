...
// Helper to generate a unique username for teachers
...
export async function POST(request: Request) {
  try {
    ...
    const teacher = await prisma.user.create({
      data: { name, username, password: hashedPassword, passwordPlain: plainPassword, role: "TEACHER" }
    });

    return NextResponse.json({
      success: true,
      teacher: { id: teacher.id, name: teacher.name, username: teacher.username, passwordPlain: teacher.passwordPlain },
      plainPassword
    });
  } catch (error) {
...
// PATCH - Edit teacher details or password
